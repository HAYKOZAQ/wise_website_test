...
Write a complete answer in clear English:"""
        ...
        if history:
            ...
            for turn in history[-6:]:
                ...
                lines.append(f"{label}: {content[:500]}")
            if lines:
                hist_ru = "\n\nПРЕДЫДУЩИЕ ВОПРОСЫ (только для контекста; факты — из КОНТЕКСТА):\n" + "\n".join(lines)
...
3) Пишите ПОЛНЫМИ предложениями. Никогда не обрывайте слово или список на середине. Завершайте каждый раздел.
4) Предпочитайте конкретные цифры и списки ИЗ контекста. Отдавайте предпочтение [legal] фактам перед [summary], если они противоречат. Суммы из [summary] считайте предварительными.
...
6) Объясните логику: кто → какое пособие → сколько → как работает → как подать заявление.

Обязательная структура markdown (заполните все разделы; если неизвестно — так и напишите):
## Краткий ответ
...
Գրեք ամբողջական պատասխանը պարզ հայերենով՝"""
    ...
    def generate_response(
        ...
        if not answer:
            if user_lang == "en":
                ...
                answer = (
                    "К сожалению, соответствующая информация не найдена. "
                    "Пожалуйста, позвоните на горячую линию Единой социальной службы 114 или посетите e-soc.am."
                )
            else:
                answer = (
        ...
        if mode in ("gemini", "ollama") and is_answer_incomplete(answer) and relevant:
            ...
            if len(answer) < 400:
                ...
                if user_lang == "en":
                    ...
                    answer = (
                        answer.rstrip()
                        + "\n\n---\n\n## Полные сведения из официальных материалов\n\n"
                        + extract
                    )
                else:
                    answer = (
    ...
    def _extractive_answer(
        ...
        if user_lang == "ru":
            ru_parts = [
                "## Краткий ответ",
                f"Из официальных материалов по вашему вопросу (**«{query}»**):",
                "",
                primary,
                "",
                "## Как работает программа",
                "Смотрите правила и шаги в материалах выше. Детали могут зависеть от вашей конкретной ситуации.",
                "",
                "## Как и куда обратиться",
                "- Онлайн: **e-soc.am**",
                "- Лично: территориальный центр Единой социальной службы (ЕСС)",
                "- Горячая линия: **114**",
            ]
            if extra_blocks:
                ru_parts += ["", "## Дополнительные официальные выдержки", ""] + [
                    b + "\n" for b in extra_blocks
                ]
            ru_parts += [
                "",
